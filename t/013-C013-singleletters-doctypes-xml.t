...
BEGIN { 
    ...
    my (%out) = BuildIndex::build_index_from_directory( 
        "data/$base-singleletters-doctypes-xml", 
        "blib/index/$base.index",
        #"conf/stemming-libxml2.conf",   # use the basic stemming configuration
        "conf/stemming-idf-libxml2-doctypeproperty.conf",   # use the basic stemming configuration
        #"conf/compressed-libxml2.conf",   # use the basic compression configuration
        #" -T INDEX_WORDS_META ",
    );

    cmp_ok( scalar(keys(%out)),     '>',    2, "Indexing output" ); 
    cmp_ok( $out{unique},     '==',   20, 'unique words indexed' );
    cmp_ok( $out{properties}, '==',   6, 'num properties' );
    cmp_ok( $out{files},      '==',   10, 'files indexed' );
    cmp_ok( $out{bytes},      '==', 1120, 'bytes indexed' );
    cmp_ok( $out{words},      '==',   20, 'total words indexed' );
    ...
    for my $letter (@letters) {
        ...
        cmp_ok(scalar(@other_rows), '==', 9, "num results from 'not $letter'" );
